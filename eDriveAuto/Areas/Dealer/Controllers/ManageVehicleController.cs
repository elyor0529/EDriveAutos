    ...
    public class ManageVehicleController : Controller
    {
        ...
        public ActionResult DealerInfo(Int32 id)
        {
            ...
            using (Edrive_ServiceClient service = new Edrive_ServiceClient())
            {
                ...
                var cust = service.GetDealerbyProductID(productId);
                ViewData["ProductID"] = productId;
                ViewData["DealerName"] = cust.Name;
                ViewData["StreetAddress1"] = cust.StreetAddress1 ?? "";
                ViewData["Phone"] = cust.Phone ?? "";

                var state = service.GetStateByCountry(1);//  ---1-- for U.S.get state Name
                ...
                ViewData["Subject"] = product.Year + " " + product.Make + " " + product.ModelName + " (VIN# " + product.vin + " )";
                return View();
            }
        }
        ...
        public ActionResult DealerInfo(_SendDealerEmail MailModel)
        {
            ...
            using (Edrive_ServiceClient service = new Edrive_ServiceClient())
            {
                ...
                Admin.Controllers.ManageVehicleController ms = new Admin.Controllers.ManageVehicleController();
               //var resul= ms.DealerInfo(MailModel);
